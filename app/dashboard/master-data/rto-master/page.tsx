...
import { INDIAN_RTOS } from '@/data/indian-rtos'

const rtoSchema = z.object({
  rtoId: z.string().min(1, 'RTO ID is required'),
  cityId: z.string().min(1, 'City ID is required'),
  rtoName: z.string().min(1, 'RTO Name is required'),
  vehicleCarrierTypeId: z.string().min(1, 'Vehicle Carrier Type ID is required'),
  rtoDescription: z.string().optional(),
  isActive: z.boolean(),
...
type RTOForm = z.infer<typeof rtoSchema>
...
export default function RTOMasterPage() {
  const router = useRouter()
  const [rtos, setRtos] = useState<RTO[]>([])
  const [filteredRtos, setFilteredRtos] = useState<RTO[]>([])
  const [searchTerm, setSearchTerm] = useState('')
  const [showModal, setShowModal] = useState(false)
  const [editingRto, setEditingRto] = useState<RTO | null>(null)
  ...
  useEffect(() => {
    ...
    if (searchTerm) {
      filtered = filtered.filter(rto =>
        rto.rtoName.toLowerCase().includes(searchTerm.toLowerCase()) ||
        rto.rtoId.toLowerCase().includes(searchTerm.toLowerCase()) ||
        rto.cityId.toLowerCase().includes(searchTerm.toLowerCase()) ||
        rto.rtoDescription?.toLowerCase().includes(searchTerm.toLowerCase())
      )
    }

    setFilteredRtos(filtered)
  }, [rtos, searchTerm])
  ...
  const handleAddRto = () => {
    ...
    reset({
      ...
      createdProcName: 'PROC_RTO_INSERT',
      updatedBy: '',
      updatedDate: '',
  ...
  const handleEditRto = (rto: RTO) => {
    setEditingRto(rto)
    reset({
      rtoId: rto.rtoId,
      cityId: rto.cityId,
      rtoName: rto.rtoName,
      vehicleCarrierTypeId: rto.vehicleCarrierTypeId,
      rtoDescription: rto.rtoDescription || '',
      isActive: rto.isActive,
      createdBy: rto.createdBy,
      createdDate: rto.createdDate,
      createdProcName: rto.createdProcName,
      updatedBy: rto.updatedBy || '',
      updatedDate: rto.updatedDate || '',
      activeFromDate: rto.activeFromDate,
      activeToDate: rto.activeToDate || '',
    })
    setShowModal(true)
  }

  const onSubmit = (data: RTOForm) => {
    if (editingRto) {
      // Update existing RTO
      setRtos(prev =>
        prev.map(rto =>
          rto.id === editingRto.id
            ? {
                ...rto,
                ...data,
      ...
      setRtos(prev => [...prev, newRto])
      toast.success('RTO added successfully!')
    }
    setShowModal(false)
    reset()
  }

  const handleDeleteRto = (id: string) => {
    if (confirm('Are you sure you want to delete this RTO?')) {
      setRtos(prev => prev.filter(rto => rto.id !== id))
      toast.success('RTO deleted successfully!')
    }
  }

  const totalPages = Math.ceil(filteredRtos.length / itemsPerPage)
  const startIndex = (currentPage - 1) * itemsPerPage
  const endIndex = startIndex + itemsPerPage
  const currentRtos = filteredRtos.slice(startIndex, endIndex)
    ...
    <div className="space-y-6">
        ...
        <nav className="flex" aria-label="Breadcrumb">
          <ol className="flex items-center space-x-4">
            ...
            <li>
              <div className="flex items-center">
                <span className="text-gray-400">/</span>
                <span className="ml-4 text-gray-900 font-medium">RTO Master</span>
              </div>
            </li>
          </ol>
        </nav>
      ...
      <div className="flex justify-between items-center">
        <div className="flex items-center space-x-4">
          ...
          <div>
            <h1 className="text-lg font-semibold text-gray-900">RTO Master</h1>
            <p className="mt-1 text-sm text-gray-600">
              Manage Regional Transport Office information
            </p>
          </div>
        </div>
        <button
        ...
        <div className="overflow-x-auto">
          <table className="min-w-full divide-y divide-gray-200">
            ...
            <tbody className="bg-white divide-y divide-gray-200">
              {currentRtos.map((rto) => (
                <tr key={rto.id} className="hover:bg-gray-50">
                  ...
                  <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-900">
                    {rto.vehicleCarrierTypeId}
                  </td>
                  <td className="px-6 py-4 whitespace-nowrap">
                    <span className={`inline-flex px-2 py-1 text-xs font-semibold rounded-full ${
                      rto.isActive ? 'bg-green-100 text-green-800' : 'bg-red-100 text-red-800'
                    }`}>
                      {rto.isActive ? 'Active' : 'Inactive'}
                    </span>
                  </td>
                  <td className="px-6 py-4 whitespace-nowrap text-sm font-medium">
                    <div className="flex space-x-2">
                      <button
                        onClick={() => handleEditRto(rto)}
                      ...
                      <button
                        onClick={() => handleDeleteRto(rto.id)}
            ...
            <div className="hidden sm:flex-1 sm:flex sm:items-center sm:justify-between">
              <div>
                <p className="text-sm text-gray-700">
                  ...
                  <span className="font-medium">{Math.min(endIndex, filteredRtos.length)}</span> of{' '}
                  <span className="font-medium">{filteredRtos.length}</span> results
                </p>
              </div>
              <div>
          ...
          <div className="relative top-20 mx-auto p-5 border w-full max-w-4xl shadow-lg rounded-md bg-white">
            <div className="mt-3">
              <h3 className="text-lg font-medium text-gray-900 mb-4">
                {editingRto ? 'Edit RTO' : 'Add New RTO'}
              </h3>
              <form onSubmit={handleSubmit(onSubmit)} className="space-y-4">
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                  <div>
                    <label className="block text-sm font-medium text-gray-700 mb-1">RTO ID *</label>
                    <input
                      {...register('rtoId')}
                      type="text"
                      className="w-full px-3 py-2 border border-gray-300 rounded-md focus:outline-none focus:ring-2 focus:ring-blue-500"
                    />
                    {errors.rtoId && <p className="text-red-500 text-xs mt-1">{errors.rtoId.message}</p>}
                  </div>
                  <div>
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                  <div>
                    ...
                    <input
                      ...
                      className="w-full px-3 py-2 border border-gray-300 rounded-md focus:outline-none focus:ring-2 focus:ring-blue-500"
                    />
                    {errors.rtoName && <p className="text-red-500 text-xs mt-1">{errors.rtoName.message}</p>}
                  </div>
                  <div>
                ...
                <div>
                  <label className="block text-sm font-medium text-gray-700 mb-1">RTO Description</label>
                  <textarea
                    {...register('rtoDescription')}
                    rows={3}
                    className="w-full px-3 py-2 border border-gray-300 rounded-md focus:outline-none focus:ring-2 focus:ring-blue-500"
                  />
                </div>
