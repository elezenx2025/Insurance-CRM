...
const fuelTypeSchema = z.object({
  ...
  fuelTypeName: z.string().min(1, 'Fuel Type Name is required'),
...
type FuelTypeForm = z.infer<typeof fuelTypeSchema>
...
export default function FuelTypeMasterPage() {
  ...
  const [filteredFuelTypes, setFilteredFuelTypes] = useState<FuelType[]>([])
  ...
  const {
    ...
    reset,
    formState: { errors },
  } = useForm<FuelTypeForm>({
    resolver: zodResolver(fuelTypeSchema),
  })

  // Mock data
  useEffect(() => {
    const mockData: FuelType[] = [
      {
        id: '1',
        fuelTypeId: 'FT001',
        fuelTypeName: 'Petrol',
        fuelTypeDescription: 'Gasoline fuel for internal combustion engines',
        isActive: true,
        createdBy: 'Admin',
        createdDate: '2024-01-01',
        createdProcName: 'PROC_FUEL_TYPE_INSERT',
        updatedBy: 'Admin',
      ...
      },
      {
        ...
        fuelTypeId: 'FT002',
        fuelTypeName: 'Diesel',
        fuelTypeDescription: 'Diesel fuel for compression ignition engines',
        isActive: true,
        createdBy: 'Admin',
        createdDate: '2024-01-01',
        createdProcName: 'PROC_FUEL_TYPE_INSERT',
      ...
      },
      {
        ...
        fuelTypeId: 'FT003',
        fuelTypeName: 'CNG',
        fuelTypeDescription: 'Compressed Natural Gas',
        isActive: true,
        createdBy: 'Admin',
        createdDate: '2024-01-01',
        createdProcName: 'PROC_FUEL_TYPE_INSERT',
      ...
      },
      {
        ...
        fuelTypeId: 'FT004',
        fuelTypeName: 'LPG',
        fuelTypeDescription: 'Liquefied Petroleum Gas',
        isActive: true,
        createdBy: 'Admin',
        createdDate: '2024-01-01',
        createdProcName: 'PROC_FUEL_TYPE_INSERT',
      ...
      },
      {
        ...
        fuelTypeDescription: 'Electric vehicle power source',
        isActive: true,
        createdBy: 'Admin',
        createdDate: '2024-01-01',
        createdProcName: 'PROC_FUEL_TYPE_INSERT',
    ...
    setFuelTypes(mockData)
  }, [])

  // Filter data based on search term
  useEffect(() => {
    let filtered = fuelTypes

    if (searchTerm) {
      filtered = filtered.filter(fuelType =>
        fuelType.fuelTypeName.toLowerCase().includes(searchTerm.toLowerCase()) ||
        fuelType.fuelTypeId.toLowerCase().includes(searchTerm.toLowerCase()) ||
        fuelType.fuelTypeDescription?.toLowerCase().includes(searchTerm.toLowerCase())
      )
    }

    setFilteredFuelTypes(filtered)
  }, [fuelTypes, searchTerm])
  ...
  const handleEditFuelType = (fuelType: FuelType) => {
    ...
    reset({
      ...
      createdDate: fuelType.createdDate,
      createdProcName: fuelType.createdProcName,
      updatedBy: fuelType.updatedBy || '',
      updatedDate: fuelType.updatedDate || '',
      activeFromDate: fuelType.activeFromDate,
      activeToDate: fuelType.activeToDate || '',
    })
    setShowModal(true)
  }

  const onSubmit = (data: FuelTypeForm) => {
    if (editingFuelType) {
      // Update existing fuel type
      setFuelTypes(prev =>
        prev.map(fuelType =>
          fuelType.id === editingFuelType.id
            ? {
                ...fuelType,
                ...data,
  ...
  const handleDeleteFuelType = (id: string) => {
    if (confirm('Are you sure you want to delete this fuel type?')) {
      setFuelTypes(prev => prev.filter(fuelType => fuelType.id !== id))
    ...
    <div className="space-y-6">
        ...
        <nav className="flex" aria-label="Breadcrumb">
          <ol className="flex items-center space-x-4">
            ...
            <li>
              <div className="flex items-center">
                <span className="text-gray-400">/</span>
                <span className="ml-4 text-gray-900 font-medium">Fuel Type Master</span>
              </div>
            </li>
          </ol>
        </nav>
      ...
      <div className="flex justify-between items-center">
        <div className="flex items-center space-x-4">
          ...
          <div>
            <h1 className="text-lg font-semibold text-gray-900">Fuel Type Master</h1>
            <p className="mt-1 text-sm text-gray-600">
              Manage vehicle fuel type information
            </p>
          </div>
        </div>
        <button
      ...
      <div className="bg-white p-4 rounded-lg shadow-sm border border-gray-100">
        <div className="flex items-center space-x-4">
          ...
          <div className="text-sm text-gray-500">
            {filteredFuelTypes.length} fuel types
          </div>
        </div>
      </div>
                ...
                <tr key={fuelType.id} className="hover:bg-gray-50">
                  ...
                  <td className="px-6 py-4 whitespace-nowrap">
                    <span className={`inline-flex px-2 py-1 text-xs font-semibold rounded-full ${
                      fuelType.isActive ? 'bg-green-100 text-green-800' : 'bg-red-100 text-red-800'
                    }`}>
                      {fuelType.isActive ? 'Active' : 'Inactive'}
                    </span>
                  </td>
                  <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-500">
                    {fuelType.activeFromDate} - {fuelType.activeToDate || 'Present'}
                  </td>
            ...
            <div className="hidden sm:flex-1 sm:flex sm:items-center sm:justify-between">
              <div>
                <p className="text-sm text-gray-700">
                  ...
                  <span className="font-medium">{Math.min(endIndex, filteredFuelTypes.length)}</span> of{' '}
                  <span className="font-medium">{filteredFuelTypes.length}</span> results
                </p>
              </div>
              <div>
            ...
            <div className="mt-3">
              ...
              <form onSubmit={handleSubmit(onSubmit)} className="space-y-4">
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                  ...
                  <div>
                    <label className="block text-sm font-medium text-gray-700 mb-1">Fuel Type Name *</label>
                    <input
                      {...register('fuelTypeName')}
                      type="text"
                      className="w-full px-3 py-2 border border-gray-300 rounded-md focus:outline-none focus:ring-2 focus:ring-blue-500"
                    />
                    {errors.fuelTypeName && <p className="text-red-500 text-xs mt-1">{errors.fuelTypeName.message}</p>}
                  </div>
                </div>

                <div>
                  <label className="block text-sm font-medium text-gray-700 mb-1">Fuel Type Description</label>
                  <textarea
                    {...register('fuelTypeDescription')}
                    rows={3}
                    className="w-full px-3 py-2 border border-gray-300 rounded-md focus:outline-none focus:ring-2 focus:ring-blue-500"
                  />
                </div>
                ...
                <div className="flex justify-end space-x-3 pt-4">
                  ...
                  >
                    {editingFuelType ? 'Update' : 'Add'} Fuel Type
                  </button>
                </div>
              </form>
            </div>
