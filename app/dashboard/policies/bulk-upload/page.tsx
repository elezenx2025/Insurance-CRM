...
export default function PoliciesBulkUploadPage() {
  const router = useRouter()
  const [showUploadModal, setShowUploadModal] = useState(false)
  const [uploadHistory, setUploadHistory] = useState([
    {
      id: '1',
      fileName: 'policies_batch_1.xlsx',
      uploadedAt: '2024-01-15T10:30:00Z',
      status: 'completed',
      recordsProcessed: 150,
      recordsSuccessful: 148,
      recordsFailed: 2,
      errors: ['Row 45: Invalid policy number format', 'Row 78: Missing required field']
    },
    {
      id: '2',
      fileName: 'policies_batch_2.xlsx',
      uploadedAt: '2024-01-14T14:20:00Z',
      status: 'completed',
      recordsProcessed: 200,
      recordsSuccessful: 200,
      recordsFailed: 0,
      errors: []
    },
    {
      id: '3',
      fileName: 'policies_batch_3.xlsx',
  ...
  const downloadTemplate = () => {
    try {
      ...
      link.href = '/templates/policy-bulk-upload-template.csv'
      link.download = 'policy-bulk-upload-template.csv'
      link.target = '_blank'
    ...
    <div className="space-y-6">
      ...
      <div className="mb-6">
        <nav className="flex" aria-label="Breadcrumb">
          <ol className="flex items-center space-x-4">
            <li>
              <button onClick={() => router.push('/dashboard/policies')} className="text-gray-400 hover:text-gray-500">
                Dashboard
              </button>
            </li>
            <li>
              <div className="flex items-center">
                ...
                <button onClick={() => router.push('/dashboard/policies')} className="ml-4 text-gray-400 hover:text-gray-500">
                  Policies
                </button>
              </div>
            </li>
      ...
      <div className="flex justify-between items-center">
        <div className="flex items-center space-x-4">
          ...
          <div>
            <h1 className="text-lg font-semibold text-gray-900">Policies Bulk Upload</h1>
            <p className="mt-1 text-sm text-gray-600">
              Upload multiple policies at once using Excel files
            </p>
          </div>
        </div>
        <button
      ...
      <div className="bg-blue-50 border border-blue-200 rounded-lg p-6">
        ...
        <div className="space-y-3 text-sm text-blue-800">
          ...
          <div className="flex items-start">
            ...
            <span>Fill in the policy data following the template structure</span>
          </div>
          <div className="flex items-start">
      ...
      <BulkUploadModal
        ...
        description="Upload an Excel file containing policy data. The file should follow the template format."
        acceptedFileTypes={['.csv', '.xlsx', '.xls']}
        maxFileSize={10}
        onUpload={handleUpload}
        templateUrl="/templates/policy-bulk-upload-template.csv"
      />
    </div>
  )
}
